...
pub const channel = struct {
    ...
    fn Up(comptime exclusive_access: AnyLock, comptime barrierFn: MemoryBarrierFn) type {
        return extern struct {
            ...
            /// written.
            fn writeAvailable(self: *Self, bytes: []const u8) usize {
            ...
            fn writeBlocking(self: *Self, bytes: []const u8) usize {
                const count = bytes.len;
                var written: usize = 0;
                while (written != count) {
                    written += self.writeAvailable(bytes[written..]);
                }
                return count;
            }
            ...
            fn writeAllowDroppedData(self: *Self, bytes: []const u8) void {
                exclusive_access.lockFn(exclusive_access.context);
                defer exclusive_access.unlockFn(exclusive_access.context);
                switch (self.mode()) {
                    .NoBlockSkip => {
                        if (bytes.len <= self.availableSpace()) {
                            _ = self.writeAvailable(bytes);
                        }
                    },
                    .NoBlockTrim => {
                        _ = self.writeAvailable(bytes);
            ...
            pub const Writer = struct {
                ...
                fn drain(io_w: *std.Io.Writer, data: []const []const u8, splat: usize) std.Io.Writer.Error!usize {
                    ...
                    if (buffered.len > 0) {
                        up.writeAllowDroppedData(buffered);
                        _ = io_w.consumeAll();
                    }
                    var n: usize = 0;
                    for (data[0 .. data.len - 1]) |d| {
                        up.writeAllowDroppedData(d);
                        n += d.len;
                    }
                    for (0..splat) |_| {
                        const to_splat = data[data.len - 1];
                        up.writeAllowDroppedData(to_splat);
                        n += to_splat.len;
                    }
                    return n;
                }
    ...
    /// does not fill up the buffer in order to avoid the problem of being unable to
    /// distinguish between full and empty.
    fn Down(comptime exclusive_access: AnyLock, comptime barrierFn: MemoryBarrierFn) type {
        return extern struct {
            /// Name is optional and is not required by the spec. Standard names so far are:
            /// "Terminal", "SysView", "J-Scope_t4i4"
            name: [*:0]const u8,
            ...
            pub fn readAvailable(self: *Self, bytes: []u8) usize {
                exclusive_access.lockFn(exclusive_access.context);
                defer exclusive_access.unlockFn(exclusive_access.context);
            ...
            pub const Reader = struct {
                ...
                fn stream(io_r: *std.Io.Reader, io_w: *std.Io.Writer, limit: std.Io.Limit) std.Io.Reader.StreamError!usize {
                    ...
                    if (down.bytesAvailable() == 0) return error.EndOfStream;
                    const n = down.readAvailable(dest);
                    io_w.advance(n);
                    return n;
                }
            };
